...
class PrometheusClient:
    ...
    def get_slo_evaluations(self, metric_name="|".join(MB['parameter']), threshold=20):
    ...
    # @utils.print_execution_time  # only around 3ms
    def get_param_assignments(self, metric_name="|".join([item[0] for item in MB['slos']])):
        raw_result = self.client.custom_query(f'{{__name__=~"{metric_name}"}}')
        transformed = utils.convert_prom_multi(raw_result)
        return transformed
...
if __name__ == "__main__":
    ...
    print(client.get_param_assignments())
    print(client.get_slo_evaluations())
