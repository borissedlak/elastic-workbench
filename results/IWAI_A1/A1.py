...
from agent.RRMGlobalAgent import RRMGlobalAgent
...
def eval_scaling_agent(agent_factory, agent_type):
    ...
    for rep in range(1, EXPERIMENT_REPETITIONS + 1):
        ...
        time.sleep(
            4
        )  # Needs a couple of seconds after resetting the services (i.e., calling ES)

        agent.start()
        time.sleep(EXPERIMENT_DURATION)
        agent.terminate_gracefully()
        export_experience_buffer(
            agent.experience_buffer, ROOT + f"/agent_experience_{agent_type}.csv"
        )
        print(
            f"{agent_type} agent finished evaluation round #{rep} after {EXPERIMENT_DURATION * rep} seconds"
        )


color_dict = {
    "elastic-workbench-qr-detector-1": "red",
    "elastic-workbench-cv-analyzer-1": "green",
}
    ...
    for agent in agent_types:
        ...
        for service in df["service"].unique():
            ...
            plt.plot(
                x,
                s_mean,
                label=service + f", {agent}",
                color=color_dict[service],
                linewidth=2,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train or Evaluate DQN Agents"
    )
    parser.add_argument("--train_first", action="store_true", help="(Re-)train Agents. Overrides existing weights")
    args = parser.parse_args()
    if args.train_first:
        logger.info("(Re-)training DQN agents..")
        train_q_network()

    logger.info("Loading DQN weights..")
    ...
    agent_fact_rrm = lambda repetition: RRMGlobalAgent(
        prom_server=ps,
        services_monitored=[qr_local, cv_local],
        evaluation_cycle=EVALUATION_FREQUENCY,
        log_experience=repetition,
    ...
    # todo: this throws server errors
