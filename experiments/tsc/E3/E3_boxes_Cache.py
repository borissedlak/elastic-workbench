...
def main():
    ...
    box = ax1.boxplot(iteration_data, labels=labels, widths=[0.5] * 3)
    ...
    fig1.tight_layout()
    fig1.savefig("plots/E3_runtime_Cache.pdf")
    plt.show()
    plt.close(fig1)

    # Plot 2: SLO Fulfillment
    fig2, ax2 = plt.subplots(figsize=(4.0, 3.0))
    box2 = ax2.boxplot(slo_data, labels=labels, widths= [0.5] * 3)
    ...
    fig2.tight_layout()
    fig2.savefig("plots/E3_SLO_F_Cache.pdf")
    plt.show()
    plt.close(fig2)
