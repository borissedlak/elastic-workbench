...
from iwai.DQN_Trainer import DQN, ACTION_DIM_QR, QR_QUALITY_STEP, CV_QUALITY_STEP, ACTION_DIM_CV
...
class DQN_Agent(ScalingAgent):
    def __init__(self, prom_server, services_monitored: list[ServiceID],
    ...
    def get_optimal_local_ES(self, service: ServiceID, service_state, assigned_clients: Dict[str, int]) -> tuple[EsType, Dict]:
        ...
        if 'model_size' in service_state or 'model_size' in all_client_slos[0]:
            model_size, model_size_t = service_state['model_size'], all_client_slos[0]['model_size'].thresh

        quality_t, throughput_t = all_client_slos[0]['quality'].thresh, all_client_slos[0]['throughput'].thresh
        state_pw = Full_State_DQN(service_state['quality'], quality_t,
                                  service_state['throughput'], throughput_t,
                                  model_size, model_size_t,
                                  service_state['cores'], free_cores,
                                  boundaries)

        action_pw = self.dqn[service.service_type].choose_action(np.array(state_pw.for_tensor()), rand=0.0)

        step_quality = QR_QUALITY_STEP if service.service_type == ServiceType.QR else CV_QUALITY_STEP
        if 1 <= action_pw <= 2:
        ...
        if 5 <= action_pw <= 6:
            ...
            return EsType.MODEL_SCALE, {'model_size': int(state_pw.model_size + delta_model_size)}
